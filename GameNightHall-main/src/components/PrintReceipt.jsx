...
export default function PrintReceipt({ bill, billItems, counterName }) {
    const totalItems = billItems.reduce((sum, item) => sum + item.quantity, 0)
    const cashierName = bill.cashierName || 'N/A'
    const counterLabel = counterName || bill.counterName || ''

    // If the bill uses the autogenerated client name (e.g. "Client ..."),
    // and the bill is a drink order, show the drink name instead.
    const isAutoClientName = bill.playerName?.startsWith('Client ')
    const drinkItem = billItems.find(i => i.itemType === 'drink')
    const displayClientName = !isAutoClientName || !drinkItem
        ? bill.playerName
        : drinkItem.itemName
        ...
        <div className="print-receipt">
            ...
            <div className="receipt-section">
                <div className="receipt-row">
                    <span>Client:</span>
                    <span><strong>{displayClientName}</strong></span>
                </div>
                {counterLabel && (
                    <div className="receipt-row">
                        <span>Compteur:</span>
                        <span><strong>{counterLabel}</strong></span>
                    </div>
                )}
                {bill.tableNumber && (
                    <div className="receipt-row">
